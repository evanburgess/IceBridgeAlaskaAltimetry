...
if a:
    ...
    d = GetLambData(verbose=False,interval_max=30,interval_min=5,by_column=True,as_object=True,get_hypsometry=True,longest_interval=True,removerepeats=True)    
...
gul_gau_x,gul_gau_y = loess_point_smooth(N.array(midgulx),N.array(midguly),frac=0.7)
#z = lowess(y, x)
...
ax1 = plt.subplot2grid((9,1), (7, 0), rowspan=2)
ax2 = plt.subplot2grid((9,1), (5, 0), rowspan=2)
ax3 = plt.subplot2grid((9,1), (3, 0), rowspan=2)
ax4 = plt.subplot2grid((9,1), (0, 0), rowspan=2)
...
#PLOTTING POINTS
ax1.plot_date(middate[wse],d.mb[wse],'o',alpha = 0.5)
ax2.plot_date(middate[wsc],d.mb[wsc],'o',alpha = 0.5)
ax3.plot_date(middate[win],d.mb[win],'o',alpha = 0.5)

#PLOTTING GAUSSIAN SMOOTH FUNCTIONS
ax1.plot_date(se_gau_x,se_gau_y,'k-',lw=2)
ax2.plot_date(sc_gau_x,sc_gau_y,'k-',lw=2)
ax3.plot_date(in_gau_x,in_gau_y,'k-',lw=2)
ax4.plot_date(gul_gau_x,gul_gau_y,'r-',lw=2,zorder=4)
ax5.plot_date(gul_gau_x,gul_gau_y,'r-',lw=2,zorder=4)
...
ax4.plot_date(gulx,guly,'r-',lw=0.5)
ax4.plot_date(wolx,woly,'b-',lw=0.5)
ax4.plot_date(midgulx,midguly,'ro',alpha = 0.5,zorder=3)
ax5.plot_date(midgulx,midguly,'ro',alpha = 0.5)
ax4.plot_date(midwolx,midwoly,'bo',alpha = 0.5)



trendstats =  (mk_test(se_mk, alpha = 0.05),mk_test(sc_mk, alpha = 0.05),mk_test(in_mk, alpha = 0.05),mk_test(gul_time_mk, alpha = 0.05),mk_test(gul_mk, alpha = 0.05),mk_test(wol_mk, alpha = 0.05))
abc=["E","D","C","A","B"]
...
for i,axis in enumerate(axes):
    ...
    #axis.annotate(abc[i],[datetime.date(1995,1,1),0.7],fontsize=15, fontweight='bold')
    axis.annotate(abc[i],xy=(.025, .975),xycoords='axes fraction',fontsize=15, fontweight='bold',va='top',ha='left')
    if i < 3:axis.annotate("Mann-Kendall p=%4.2f" % trendstats[i][1],[datetime.date(1995,1,1),-3.7],fontsize=9)
    elif i==3:axis.annotate("Mann-Kendall\nGulkana timeseries (1994-2013) p=%4.2f\n    sampled over altimetry intervals p=%5.3f\nWolverine p=%4.2f" % (trendstats[i][1],trendstats[i+1][1],trendstats[i+2][1]),[datetime.date(1994,8,1),-3.9],fontsize=9)
...
fig.savefig("/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/midpoint_timeseries_longest_interval.jpg",dpi=300)
plt.show()

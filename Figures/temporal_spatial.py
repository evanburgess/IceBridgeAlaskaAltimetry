...
def cleanboxes(box,cchoice):
    ...
    for bx in box['medians']:
        bx.set_color(color='k')
        bx.set_lw(2)
    for clr,bx in zip([x1 for pair in zip(cchoice,cchoice) for x1 in pair],box['caps']):
        bx.set_color(color=clr)
        bx.set_lw(2)
    for clr,bx in zip([x1 for pair in zip(cchoice,cchoice) for x1 in pair],box['whiskers']):
        bx.set_color(color=clr)
...
if runall:    
    ...
    pickle.dump([lamb,userwheres,notused,whereswo,notswo,altstd,starts,ends,altn], open("/Users/igswahwsmcevan/Altimetry/results/spatialtemporal.p", "wb" ))
else:
    lamb,userwheres,notused,whereswo,notswo,altstd,starts,ends,altn = pickle.load(open("/Users/igswahwsmcevan/Altimetry/results/spatialtemporal.p", "rb" ))
...
def expdecay(x,n,tau):return n*exp(-x/tau)

...
smb = d.smb.astype(float)[:]*d.area/1000
...
w = N.where(N.logical_and(flx!=0,N.logical_and(N.array(d.name)!='Columbia Glacier',N.array(d.name)!='Hubbard Glacier')))
...
cax = fig.add_axes([0.78,0.25,0.05,0.5])
...
#REGRESSION SIG
ax.text(0.7,0.4,"r=%1.2f, sig=%1.3f"%(corr[0],corr[1]),fontsize=11,transform=ax.transAxes,ha='right')
ax.text(0.1,0.94,"Negative Total Mass Balance",fontsize=11,transform=ax.transAxes,ha='left', style='italic',bbox={'facecolor':'white', 'pad':4})
...
label = ['Columbia','LeConte','Hubbard','Taku','Yahtse','Reid']
va = ['top','bottom','bottom','top','top','top','bottom']
ha = ['left','right','right','left','left','left']
...
for i in xrange(len(smb)):
    ...
    #    j+=1
ax.set_xlabel("SMB (Gt yr"+"$\mathregular{^{-1})}$",fontsize=11,labelpad=-1)
ax.set_ylabel("Calving Flux (Gt yr"+"$\mathregular{^{-1}})$",fontsize=11,labelpad=0.5)
...
plt.show()
fig.savefig("/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/flux_vs_smb3_notnorm.jpg",dpi=300)

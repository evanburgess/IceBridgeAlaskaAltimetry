...
if startfromscratch:    
 ...
 #
    ...
    interior.calc_dz_stats(too_few=2)
    interior.extend_upper_extrap()
    
    sc = GetLambData(verbose=False,longest_interval=True,interval_max=30,interval_min=5,by_column=True,as_object=True, userwhere="FLOOR((ergi.glactype::real-9000)/100)=2 AND gltype.surge='f' AND%s" % scz) 
    print sc
    sc.fix_terminus()
    sc.remove_upper_extrap(remove_bottom=True)
    sc.normalize_elevation()
    sc.calc_dz_stats(too_few=2)
    sc.extend_upper_extrap()

    se = GetLambData(verbose=False,longest_interval=True,interval_max=30,interval_min=5,by_column=True,as_object=True, userwhere="FLOOR((ergi.glactype::real-9000)/100)=2 AND gltype.surge='f' AND%s" % sez) 
    #se.fix_terminus()
    se.remove_upper_extrap(remove_bottom=True)
    se.normalize_elevation()
    se.calc_dz_stats(too_few=2)

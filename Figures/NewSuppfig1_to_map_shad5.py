...
if a:
    ...
    pickle.dump([survdata,regions,d,alldata,lamb], open( "/Users/igswahwsmcevan/Desktop/temp5.p", "wb" ))
else:
    survdata,regions,d,alldata,lamb = pickle.load(open( "/Users/igswahwsmcevan/Desktop/temp5.p", "rb" ))

survdata['region']=N.where([i in ("East Yakutat Glacier","West Yakutat Glacier","Battle Glacier") for i in survdata['name']],"adsf",survdata['region'])
...
for j,i in enumerate(survdata['zone']):
    ...
    survdata['zone'][j] = re.sub('Aluetian Range','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Brooks Range','Interior',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Chugach Range','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Coast Range BC','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Fairweather Glacier Bay','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Juneau Icefield','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Kenai Mountains','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('St. Elias Mountains','Coastal',survdata['zone'][j])
    survdata['zone'][j] = re.sub('Stikine Icefield','Coastal',survdata['zone'][j])
...
for reg in zones:
    for gl in gltype:
        ...
        namedata.append(survdata['name'][N.where(N.logical_and(survdata['zone']==reg,survdata['gltype']==gl))])
        xs.append(xi)
        xi+=1
    xi+=2
xs = N.array(xs)
...
ax3 = fig3.add_axes([0.12,0.02,0.45,0.94],frameon=True)
ax = fig3.add_axes([0.7,0.02,0.25,0.94],frameon=True)
...
ax3.set_ylim(ylim)
ax3.set_xlim([ax3.get_xlim()[0]-1,ax3.get_xlim()[1]+1])
...
ax3.annotate('A',[0.3,-3.8],horizontalalignment='center',verticalalignment='center',fontproperties=font)
ax.annotate('B',[-0.3,-74],horizontalalignment='center',verticalalignment='center',fontproperties=font, annotation_clip=False)
...
plt.show()
fig3.savefig("/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/NewSuppfig1_to_map_shad5.jpg",dpi=300)
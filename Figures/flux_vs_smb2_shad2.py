...
ax = fig.add_axes([0.15,0.005,0.8,0.98])##############################################
...
cax, kw = clbr.make_axes(ax,location='bottom',pad=0.1)##############################################
clr = clbr.Colorbar(cax,colorbarim1,orientation='horizontal',ticks=N.arange(-1,1.1,0.5)) 
clr.set_label('Terminus Position Change (km year'+"$\mathregular{^{-1}})$")
...
#REGRESSION SIG
ax.text(0.93,0.68,"r=%1.2f, sig=%1.3f"%(corr[0],corr[1]),fontsize=9,transform=ax.transAxes,ha='right')



label = ['Columbia','LeConte','Tsaa','Hubbard','Taku','Margerie','Tyndall','Yahtse']
va = ['top','bottom','top','bottom','top','top','bottom','top']
ha = ['left','right','left','left','left','left','left','left']
...
for i in xrange(len(smb)):
    if re.sub(' Glacier','',d.name[i]) in label:
        ...
        ax.text(smb[i]+ho, flx[i]+vo,re.sub(' Glacier','',d.name[i]),fontsize=7,va=va[w],ha=ha[w])
    ...
    #    j+=1
ax.set_xlabel("SMB (m w eq yr"+"$\mathregular{^{-1})}$",fontsize=9,labelpad=-1)
ax.set_ylabel("Norm. Calving Flux (m w eq yr"+"$\mathregular{^{-1}})$",fontsize=9,labelpad=0.5)

for label in ax.xaxis.get_ticklabels():label.set_size(8)
for label in ax.yaxis.get_ticklabels():label.set_size(8)

...
if myr:
    ...
    flxerr = d.eb_bm_err.astype(float)*1000./d.area
else:
    balmodel = d.rlt_totalgt
    balerr = N.array(d.rlt_singlerrkgm2).astype(float)
    flx = -d.eb_bm_flx.astype(float)
    flxerr = d.eb_bm_err.astype(float)

# comverting to std from iqr  altimetry error
#balerr = (bal75diff-bal25diff)/1.394

land = GetLambData(longest_interval=True,userwhere="ergi.gltype='0'", as_object=True,results=True)
...
print scipy.stats.mstats.kruskalwallis(smb,land.rlt_totalkgm2)
# print N.extract(~N.isnan(smb),smb)
print scipy.stats.wilcoxon(N.extract(~N.isnan(smb),smb))
print scipy.stats.mstats.normaltest(N.extract(~N.isnan(smb),smb))
...
if myr:bar2 = ax2.bar(N.array(flxlocs)+0.2,-flxs['eb_bm_flx']*1000/flxs['area'],color='b',width=0.2,ecolor='k',yerr=-flxs['eb_bm_err']*1000/flxs['area'],zorder=1)
else:bar2 = ax2.bar(N.array(flxlocs)+0.2,-flxs['eb_bm_flx'],color='b',width=0.2,ecolor='k',yerr=-flxs['eb_bm_err'],zorder=1)
...
# plt.show()
if myr:outfile = "/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/tidewater2_partition_myr2_test.jpg"
else:outfile = "/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/tidewater2_partition_gt2_test.jpg"

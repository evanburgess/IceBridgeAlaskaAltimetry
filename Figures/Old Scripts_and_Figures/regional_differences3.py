...
box = ax3.boxplot(boxdata,positions=xs,widths=1,sym='',patch_artist=True,whis=[5,95])
...
for i,bx in enumerate(pointdata):
    sca = ax3.scatter(pointxs[i],bx,zorder=4, lw=1,facecolors='none',edgecolor=colorlistdark[i],s=35)
    #sca.set_lw(0)
...
braceys=[-2,-2,-2,-2.5,-2.5,-2.35,-2,-2]
begs=N.arange(0,37,5)-0.5
ends = N.arange(0,37,5)+2.5
for by,beg,end,reg in zip(braceys,begs,ends,regionslab):plot_brace(ax3,beg,end,by,0.2,up=False,annotate=reg,fontsize=10,zorder=4)
braceys=[-3.2,-3,-3]
begs = begs[[0,2,5]]
ends = ends[[1,4,7]]
...
for by,beg,end,reg in zip(braceys,begs,ends,zones):plot_brace(ax3,beg,end,by,0.2,up=False,annotate=reg,fontsize=10,bbox='white',zorder=4)
...
ax3.annotate('A',[0.5,0.2],horizontalalignment='center',verticalalignment='center',fontproperties=font)
ax.annotate('B',[-1.2,-6],horizontalalignment='center',verticalalignment='center',fontproperties=font, annotation_clip=False)
...
plt.show()
fig3.savefig("/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/NewSuppfig1_to_map.jpg",dpi=300)
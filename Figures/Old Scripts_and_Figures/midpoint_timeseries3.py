...
def rlm_date(x,y):
    ...
    #print rlmresults.pvalues
    #print rlmresults.tvalues
...
for i,axis in enumerate(axes):
    ...
    if i < 3:axis.annotate("p=%4.2f" % trendstats[i][1],[datetime.date(1995,1,1),-3.7],fontsize=9)
    #if i < 3:axis.annotate("p=%4.2f" % ps[i],[datetime.date(1995,1,1),-3.7],fontsize=9)

    elif i==3:axis.annotate("Gulkana p=%4.2f\nWolverine p=%4.2f" % (trendstats[i][1],trendstats[i+2][1]),[datetime.date(1994,8,1),-3.9],fontsize=9)
...
fig.savefig("/Users/igswahwsmcevan/Papers/AK_altimetry/Figures/midpoint_timeseries_longest_interval_lowess.jpg",dpi=300)
plt.show()

...
ax.fill_between(land.norme,land.dzs_mean+land.dzs_std,land.dzs_mean-land.dzs_std,alpha=alphafill,color= N.array([163,100,57])/255.,lw=1)
...
ax.fill_between(lake.norme,lake.dzs_mean+lake.dzs_std,lake.dzs_mean-lake.dzs_std,alpha=alphafill,color= 'green',lw=1)
...
for j,dz in enumerate(tide.normdz):
    if switch[j]: ax2.plot(tide.norme,dz,'-k',linewidth=0.3,alpha=0.3)   # can make these a separate color
    else: ax2.plot(tide.norme,dz,'-k',linewidth=0.3,alpha=0.3)

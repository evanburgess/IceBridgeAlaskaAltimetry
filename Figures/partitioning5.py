...
#barlocs = [0,1,4,5,2,3]
#
#b1,b2,b3 = ax.bar([3.2,4.2,5.2],[smb,flx,vol],yerr=[smberr,flxerr,volerr],width=width,color=[0.5,0.5,0.5],ecolor='k') # 
#
##print 'all'
...
#
#b1,b2,b3 = ax.bar(N.array([3.2,4.2,5.2])+width,[smb,flx,vol],yerr=[smberr,flxerr,volerr],width=width,color=[0.8,0.8,0.8],ecolor='k') # 
##print smb, N.sum(t['net'][w],lamb.eb_bm_flx.astype(float)[w],t['net'][w]+lamb.eb_bm_flx.astype(float)[w])
##print 'no col'
##temp =  N.c_[lamb.name[w],t['net'][w],lamb.eb_bm_flx.astype(float)[w]]
##for li in temp:print "%s,%s,%s" % (li[0],li[1],li[2])
...
barlocs = [0,0+width,2,2+width,1,1+width]
...
ax.set_ylabel("Gt a"+"$\mathregular{^{-1}}$",fontsize=11)
...
ax2.set_ylabel("Mass Balance (m w. eq. a"+"$\mathregular{^{-1})}$",fontsize=11)
